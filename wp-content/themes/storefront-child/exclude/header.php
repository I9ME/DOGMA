 ...
 */
	...
	/*function subMenu(count){

		jQuery('header .mainNav .item').siblings().removeClass('toggleOn');
		jQuery('header #count_'+count).toggleClass('toggleOn');
		//alert('OK '+count);
...
jQuery(function(){
	...
	jQuery(".subHover a.title.level-1").on("click", function (event) {
    event.stopPropagation();
    
    var id = jQuery(this).attr('id');
    
    jQuery('header #count_'+id).toggleClass('toggleOn');

    var siblings = jQuery(this).parents().siblings('li.subHover');
    
    siblings.removeClass('toggleOn');	

});

	jQuery(window).on("click", function () {
    	jQuery('.item.subHover').removeClass('toggleOn');
	});
...
<ul class="navItems vMiddle">
    ...
    foreach( $menuitems as $item ):
        ...
        $title = $item->title;
        $classCSS = $item->classes[0];
        if( $classCSS == 'subHover') {

        	//$link = 'javascript:subMenu(\'' . $count . '\');';
        	$link = 'javascript:;';
        	$inputClass = 'subHover';
        	//$link = $link_;
    ...
    <li id="count_<?php echo $count; ?>" class="item<?php if($classCSS) { echo ' ' . $inputClass; } ?>">
        <a id="<?php echo $count; ?>" href="<?php echo $link; ?>" class="title level-1"><span><?php echo $title; ?></span></a>
    <?php endif; ?>
